...
class SRData(data.Dataset):
    def __init__(self, args, name='', train=True, benchmark=False):
        ...
        data_range = [r.split('-') for r in args.data_range.split('/')]   #data_range[0] = training data set , data_range[1] = test dataset
        if train:
        ...
        self.begin, self.end = list(map(lambda x: int(x), data_range))   # begin : training or test dataset 시작, end : trainig or dataset 끝
        self._set_filesystem(args.dir_data)     # def _set_filesystem 밑에 있음,
    ...
    def _set_filesystem(self, dir_data):       #dir_data = ../data
        self.apath = os.path.join(dir_data, self.name)    #../data/RainHeavy or rainheavytest
